...
WTF.fn = WTF.PRESENT ? 
    function(p, scopefn) {
        var scope = scopefn()
        var result = p.apply(null, arguments)
        return WTF.trace.leaveScope(scope, result)
    }
: function(p) { return (typeof p == 'function' ? p() : p) }

WTF.trace.node.start = WTF.PRESENT ?
function(a) {
  wtf.trace.prepare(a);
  wtf.trace.start(a);
...
if (!WTF.PRESENT && process.env.WTF)
    throw new Error('wtf failed to load')
